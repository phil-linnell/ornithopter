...
export function process(property, tokens, fn) {
  ...
  if (isString(fn)) {
    return stepsTransform(property, tokens, fn);
  }
  return stepsSimple(property, tokens);
}
...
function stepsSimple(property, tokens) {
...
function keyframeTransform(step, property, fn, value) {
  return {step: step, properties: [{property: property, value: fn + "(" + value + ")"}]};
}

function stepsTransform(property, tokens, fn) {
  const size = tokens.length - 1;
  const stepSize = (1 / size) * 100;
  const steps = [];
  let n = 0;
  ...
  for (let i = 0; i <= size; i++) {
    const step = Math.round(n * 100) / 100;
    const value = tokens[i];
    steps.push(keyframeTransform(step, property, fn, value));
    n = n + stepSize;
  }
