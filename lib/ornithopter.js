 ...
 *
 */
export function processTransform(property, type, tokens) {
  if (!isString) {
    throw new Error("No property");
  }
  if (tokens.length < 2) {
  ...
  return stepsTransform(property, type, tokens);
};


function keyframeTransform(step, property, type, value) {
  return {step: step, properties: [{property: property, value: type + "(" + value + ")"}]};
}

function stepsTransform(property, type, tokens) {
  const size = tokens.length - 1;
  const stepSize = (1 / size) * 100;
  const steps = [];
  let n = 0;
  ...
  for (let i = 0; i <= size; i++) {
    const step = Math.round(n * 100) / 100;
    const value = tokens[i];
    steps.push(keyframeTransform(step, property, type, value));
    n = n + stepSize;
  }
