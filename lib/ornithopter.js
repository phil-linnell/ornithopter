...
import { splitByTime, toMilliseconds, keyframePercentage, splitBySmoothness } from 'utils/time';
...
function steps(property, value, fn, totalDuration) {
  const [tokens, timeComponentWithSmoothness] = splitByTime(value);
  const [timeComponent, smoothness] = splitBySmoothness(timeComponentWithSmoothness, 0);
  ...
  const amountOfPoints = smoothness + 2; // 2 means start and end points
  const placeholder = Array.apply(null, Array(amountOfPoints)).map(() => 0);

  if (isColor(tokens[0])) {
    return placeholder.reduce(interpolate(tokens.map(Color), amountOfPoints, S.valueByLinearColor), [])
                      .map(toKeyframe(property, fn, duration, delay, totalDuration, fromColor));
  }

  return placeholder.reduce(interpolate(tokens.map(toNumber), amountOfPoints, S.valueByLinearNumber), [])
                    .map(toKeyframe(property, fn, duration, delay, totalDuration, fromNumber));
}

function interpolate(tokens, amountOfPoints, timingFunctionValue) {
