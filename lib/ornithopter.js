 ...
 */
export function process(name, tokens) {
  if (!isString) {
...
export function keyframe(step, name, value) {
  return {step: step, properties: [{name: name, value: value}]};
}

export function steps(name, tokens) {
  ...
  for (let i = 0; i <= size; i++) {
    ...
    steps.push(keyframe(step, name, value));
    n = n + stepSize;
  }
