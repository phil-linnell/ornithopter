...
import {isColor, Color} from "utils/color";
...
function steps(property, value, fn, totalDuration) {
  ...
  if (isColor(tokens[0])) {
    return placeholder.map(interpolate(tokens.map(Color), accuracy, S.valueByLinearColor))
                      .map(toKeyframe(property, fn, accuracy, S.valueByLinearColor, duration, delay, totalDuration, fromColor));
  }

  return placeholder.map(interpolate(tokens.map(toNumber), accuracy, S.valueByLinear))
                    .map(toKeyframe(property, fn, accuracy, S.valueByLinear, duration, delay, totalDuration, fromNumber));
}

function interpolate(tokens, accuracy, timingFunction) {
  return (emptyValue, position, collection) => {
    const prevPosition = position === 0 ? 0 : position - 1;

    return S.interpolateValue(position, collection[prevPosition], tokens, accuracy, timingFunction);
  }
}
