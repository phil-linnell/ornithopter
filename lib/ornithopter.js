...
function steps(property, value, fn, totalDuration) {
  ...
  const [timeComponent, smoothness] = splitBySmoothness(timeComponentWithSmoothness, tokens.length - 1);
  ...
  const placeholder = Array.apply(null, Array(smoothness + 1)).map(() => 0);

  if (isColor(tokens[0])) {
    return placeholder.reduce(interpolate(tokens.map(Color), smoothness, S.valueByLinear, S.valueByLinearColor), [])
                      .map(toKeyframe(property, fn, duration, delay, totalDuration, fromColor));
  }

  return placeholder.reduce(interpolate(tokens.map(toNumber), smoothness, S.stepByLinear, S.valueByLinearNumber), [])
                    .map(toKeyframe(property, fn, duration, delay, totalDuration, fromNumber));
}

function interpolate(tokens, smoothness, timingFunctionStep, timingFunctionValue) {
