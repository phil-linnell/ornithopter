...
function steps(property, value, fn, totalDuration) {
  ...
  if (isColor(tokens[0])) {
    return placeholder.reduce(interpolate(tokens.map(Color), accuracy, S.valueByLinearColor), [])
                      .map(toKeyframe(property, fn, accuracy, S.valueByLinear, duration, delay, totalDuration, fromColor));
  }

  return placeholder.reduce(interpolate(tokens.map(toNumber), accuracy, S.valueByLinearNumber), [])
                    .map(toKeyframe(property, fn, accuracy, x => x, duration, delay, totalDuration, fromNumber));
}

function interpolate(tokens, accuracy, timingFunction) {
  return (accumulator, emptyValue, position) => {
    const prevPosition = position === 0 ? 0 : position - 1;

    return accumulator.concat(S.interpolateValue(position, accumulator[prevPosition], tokens, accuracy, timingFunction));
  }
}

function toKeyframe(property, fn, accuracy, timingFunction, duration, delay, totalDuration, stringify) {
  console.log(property, fn, accuracy, duration, delay, totalDuration);
  return (currentValue, position) => {
    const step = S.percentage(S.changePoint(position, accuracy, timingFunction), duration, delay, totalDuration);

    return keyframe(step, property, stringify(currentValue), fn);
  }
}
