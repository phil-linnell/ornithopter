...
import {splitByTime, toMilliseconds, keyframePercentage} from "utils/time";
...
function steps(property, value, fn, totalDuration) {
  const [tokens, timeComponent] = splitByTime(value);
  ...
  const steps = [];
  let stepValue = initialStepValue([duration, delay, timingFunction], totalDuration);

  tokens.forEach(token => {
    const step = computeStep(stepValue);
    steps.push(keyframe(step, property, token, fn));
...
export function initialStepValue([duration, delay, timingFunction], totalDuration) {
  if (delay === 0) {
    return 0;
  }
...
export function nextStepValue(duration, previousStepValue, totalDuration, stepSize) {
  return keyframePercentage(duration, totalDuration);
}
