...
import {isTime, convertToMilliseconds} from "utils/time";
  ...
  return (root, result) => {
    root.walkAtRules("ornithopter", ornirule => {
      ...
      root.walkDecls("animation", decl => {
        ...
        const totalDuration = animationValue.find(values => isTime(values));

        if (totalDuration === undefined) {
          throw ornirule.error("Animation declartions must contain a duration for ornitopter to function.")
        }

        return convertToMilliseconds(totalDuration);
      });
